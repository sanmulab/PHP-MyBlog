...
class Uploader
{
    ...
    private function getFullName()
    {
        //替换日期事件
        // $t = time();
        $uniqid = md5(uniqid(microtime(true) . mt_rand()));
        //8-4-4-4-12
        $t = substr($uniqid, 0, 8) . '-' . substr($uniqid, 8, 4) . '-' . substr($uniqid, 12, 4) . '-' . substr($uniqid, 12, 4) . '-' . substr($uniqid, 20);
